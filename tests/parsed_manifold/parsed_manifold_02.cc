...
test(const std::string &name)
{
  ...
  tria->refine_global(1);
  std::ofstream out(
    ("/tmp/" + name + std::to_string(dim) + std::to_string(spacedim) + ".msh")
      .c_str());
...
main()
{
  ...
  test<2, 2>("hyper_shell");
  test<3, 3>("hyper_shell");

  test<2, 2>("half_hyper_shell");
  test<3, 3>("half_hyper_shell");

  test<2, 2>("quarter_hyper_shell");
}
