...
test(const Triangulation<dim> &tr, const FiniteElement<dim> &fe)
{
  ...
  std::vector<SymmetricTensor<2, dim, double>> sym_grad_v_double(
    quadrature.size());
  fe_values[vector].get_function_symmetric_gradients(global_vector,
                                                     sym_grad_v_double);
      ...
      for (unsigned int d = 0; d < dim; ++d)
        {
          for (unsigned int dd = 0; dd < dim; ++dd)
            {
              const double diff =
                std::abs(t_double[d][dd] - grad_v_double[q][d][dd]);
              if (diff > 1e-10)
                deallog << "Expected 0, got: " << diff << std::endl;
            }
        }
      ...
      for (unsigned int d = 0; d < dim; ++d)
        {
          for (unsigned int dd = 0; dd < dim; ++dd)
            {
              const double diff =
                std::abs(t_double[d][dd] - sym_grad_v_double[q][d][dd]);
              if (diff > 1e-10)
                deallog << "Expected 0, got: " << diff << std::endl;
            }
        }
