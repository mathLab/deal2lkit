...
int main(int argc, char **argv)
{
  ...
  // dofs:
  std::function<unsigned int()> n_dofs = [&]() { return numprocs * ndof; };
  // create new vector:
  ...
  // residual:
  std::function<int(const VEC &, VEC &)> residual = [&](const VEC &y,
                                                        VEC &      res) {
    res = y;
    return 0;
  };
  ...
  // solve_linear_system:
  std::function<int(const VEC &, VEC &)> solve_linear_system =
    [](const VEC &, VEC &) { return 0; };
  // Jacobian vector mult:
  std::function<int(const VEC &, VEC &)> jacobian_vmult =
    [](const VEC &, VEC &) { return 0; };
  ...
  deallog << "On KinSol completion the status flag is: " << success
          << std::endl;

  // change the residual:
  std::function<int(const VEC &y, VEC &res)> new_residual = [&](const VEC &y,
                                                                VEC &res) {
