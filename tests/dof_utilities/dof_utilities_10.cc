           ...
           const FiniteElement<dim> &fe)
{
...

  {
    ...
    const std::vector<Sdouble> div_double = DOFUtilities::Val(div_v);
...

  {
    ...
    const std::vector<Tensor<1,dim,Sdouble> > t_double = DOFUtilities::Val(grad_s);
    for (unsigned int q=0; q<quadrature.size(); ++q)
      {
...

  {
    ...
    const std::vector<Tensor<2,dim,Sdouble> > t_double = DOFUtilities::Val(grad_v);
    for (unsigned int q=0; q<quadrature.size(); ++q)
      {
        for (unsigned int d=0; d<dim; ++d)
...

  {
    ...
    for (unsigned int q=0; q<quadrature.size(); ++q)
      {
        const Tensor<2,dim,Sdouble> t_double = DOFUtilities::Val(sym_grad_v[q]);
        for (unsigned int d=0; d<dim; ++d)
          for (unsigned int dd=0; dd<dim; ++dd)
