...
KINSOLInterface<VEC>::declare_parameters(ParameterHandler &prm)
{
  ...
  add_parameter(prm,
                &verbosity,
                "Level of verbosity of the KINSOL solver",
                "0",
  ...
  add_parameter(
    prm,
    &mbset,
    "Maximum number of iteration before Jacobian update",
    "10",
    Patterns::Integer(1),
  ...
  add_parameter(prm,
                &use_internal_solver,
                "Use internal KINSOL direct solver",
                "false",
                Patterns::Bool(),
...
#  endif
  ...
  // need to feed KINSOL init with a N_VECTOR. For simplicity the initial guess
...
#  endif
  ...
  Assert(status == KIN_SUCCESS,
         ExcMessage("Error initializing KINSOL. KINInit failed."));

  //   1- maximum iterations:
  status = KINSetNumMaxIters(kin_mem, max_iterations);
  Assert(status == KIN_SUCCESS,
         ExcMessage("Error initializing KINSOL. KINSetNumMaxIters failed."));

  //   2- ftol:
  status = KINSetFuncNormTol(kin_mem, ftol);
  Assert(status == KIN_SUCCESS,
         ExcMessage("Error initializing KINSOL. KINSetFuncNormTol failed."));
  ...
  Assert(status == KIN_SUCCESS,
         ExcMessage("Error intializing KINSOL. KINSetPrintLevel failed."));

  status = KINSetMaxSetupCalls(kin_mem, mbset);
  Assert(status == KIN_SUCCESS,
         ExcMessage("Error intializing KINSOL. KINSetMaxSetupCalls failed."));
...
KINSOLInterface<VEC>::solve(VEC &sol)
{
  ...
  // Check initialization of the scaling. If scaling is not initialized we
  // modify the call to spare allocating a vector.
  if (!scaling_is_set)
    {
#  ifdef DEAL_II_WITH_MPI
  ...
  if (status == KIN_MXNEWT_5X_EXCEEDED)
    {
      ...
      pcout
        << "Don't worry. This might be only a problem of scaling... Let's try."
        << std::endl;
      ...
      if (strategy == "global_newton")
        status =
          KINSol(kin_mem, this->solution, KIN_LINESEARCH, u_scale, f_scale);
    }
  AssertThrow(
    status >= 0,
    ExcMessage(
...
KINSOLInterface<VEC>::set_scaling_vectors(const VEC &uscale, const VEC &fscale)
{
  ...
  AssertThrow(
    is_initialized,
    ExcMessage(
      "KINSOLInterface is not initialized when "
