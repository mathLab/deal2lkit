...
ErrorHandler<ntables>::declare_parameters(ParameterHandler &prm)
{
  ...
  if (ntables > 1)
    {
      for (unsigned int i = 0; i < ntables - 1; ++i)
        {
          dummy_names =
            dummy_names + "error" + Utilities::int_to_string(i) + ", ";
        }
  ...
  prm.declare_entry("Table names",
                    dummy_names,
                    Patterns::List(Patterns::Anything(), ntables, ntables),
                    "Comma separated list of table names. ");
  prm.declare_entry("Error precision", "3", Patterns::Integer());
  prm.declare_entry("Solution names",
                    solution_names,
                    Patterns::Anything(),
  ...
  prm.declare_entry("Solution names for latex",
                    solution_names,
                    Patterns::Anything(),
                    "Comma separated version of the same thing as above for "
                    "the latex version of the table.");

  // prm.declare_entry ("Ib output format", "msh",
  // Patterns::Selection("raw|msh")); prm.declare_entry ("Ib input file prefix",
  // "ellipse", Patterns::Anything());
...
ErrorHandler<ntables>::parse_parameters(ParameterHandler &prm)
{
  ...
  latex_headers =
    Utilities::split_string_list(prm.get("Solution names for latex"));
      ...
      for (unsigned int i = 0; i < names.size(); ++i)
        {
          ...
          std::vector<std::string> all_comps =
            Utilities::split_string_list(all_names, ';');
              ...
              for (unsigned int k = 0; k < all_types.size(); ++k)
                {
                  if (all_types[k] == "Linfty")
                    ...
                    {
                      AssertThrow(false,
                                  ExcMessage("Didn't recognize a norm type."));
                    }
                }
            }
        }
  ...
  if (compute_error)
    {
      ...
      AssertThrow(table_no < names.size(),
      ...
      if (add_rates[table_no])
        {
          if (extras[table_no]["dofs"])
            tables[table_no].omit_column_from_convergence_rate_evaluation(
              "dofs");
          ...
          if (rate_keys[table_no] == "")
            tables[table_no].evaluate_all_convergence_rates(
              ConvergenceTable::reduction_rate_log2);
          else
            tables[table_no].evaluate_all_convergence_rates(
              rate_keys[table_no], ConvergenceTable::reduction_rate_log2);
        }

      if (output_error)
        tables[table_no].write_text(out);

      if (write_error)
        {
          std::string filename = names[table_no] + "." + error_file_format;
