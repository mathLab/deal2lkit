...
IMEXStepper<VEC>::declare_parameters(ParameterHandler &prm)
{
  ...
  add_parameter(
    prm, &abs_tol, "Absolute error tolerance", "1e-6", Patterns::Double());

  add_parameter(
    prm, &rel_tol, "Relative error tolerance", "1e-5", Patterns::Double());

  add_parameter(prm, &initial_time, "Initial time", "0.0", Patterns::Double());

  add_parameter(prm, &final_time, "Final time", "1.0", Patterns::Double());

  add_parameter(
    prm, &output_period, "Intervals between outputs", "1", Patterns::Integer());

  add_parameter(
    prm,
    &max_outer_non_linear_iterations,
  ...
  add_parameter(prm,
                &max_inner_non_linear_iterations,
                "Maximum number of inner nonlinear iterations",
                "3",
                Patterns::Integer(),
                "At each inner iteration the Jacobian is NOT updated.");

  add_parameter(
    prm, &newton_alpha, "Newton relaxation parameter", "1", Patterns::Double());

  add_parameter(prm,
                &update_jacobian_continuously,
                "Update continuously Jacobian",
                "true",
                Patterns::Bool());
  ...
  add_parameter(
    prm,
    &n_max_backtracking,
    "Number of elements in backtracking sequence",
    "5",
    Patterns::Integer(1),
  ...
  add_parameter(
    prm, &verbose, "Print useful informations", "false", Patterns::Bool());

  add_parameter(
    prm, &use_kinsol, "Use the KINSOL solver", "true", Patterns::Bool());
}
...
IMEXStepper<VEC>::solve_dae(VEC &solution, VEC &solution_dot)
{
  ...
  std::function<int(const VEC &, VEC &)> my_residual = [&](const VEC &y,
                                                           VEC &      res) {
    double a = this->get_alpha();
    compute_y_dot(y, *previous_solution, a, solution_dot);
    int ret    = this->residual(t, y, solution_dot, res);
    *residual_ = res;
    AssertThrow(!std::isnan(residual_->l2_norm()),
                ExcMessage("Residual contains one or more NaNs."));
    return ret;
  };
  ...
  if (use_kinsol)
    {
      // call kinsol initialization. this is mandatory if I am doing multiple
      // cycle in pi-DoMUS
  ...
  compute_consistent_initial_conditions(initial_time, solution, solution_dot);
  // The overall cycle over time begins here.
  while (t <= final_time + 1e-15)
    {
      pcout << "Solving for t = " << t << " (step size = " << step_size << ")"
            << std::endl;
...
template <typename VEC>
double
IMEXStepper<VEC>::line_search_with_backtracking(const VEC &   update,
  ...
  do
    {
      ...
      if (update_Jacobian == true)
        {
          setup_jacobian(t, solution, solution_dot, alpha);
        }

      inner_iter = 0;
      while (inner_iter < max_inner_non_linear_iterations &&
             res_norm > abs_tol && res_norm > rel_tol * solution_norm)
        {
          inner_iter += 1;
          ...
          *rhs *= -1.0;
          ...
          if (method == "LS_backtracking")
            ...
            {
              solution.sadd(1.0, newton_alpha, *solution_update);
            }
          ...
          AssertThrow(!std::isnan(res->l2_norm()),
                      ExcMessage("Residual contains one or more NaNs."));
          ...
          if (rel_tol > 0.0)
              ...
              if (verbose)
                {
                  pcout << std::endl
                        << "   "
                        << " iteration " << nonlin_iter + inner_iter << ":\n"
          ...
          else if (verbose)
            {
              pcout << std::endl
                    ...
                    << std::setw(19) << std::scientific << res_norm
                    << "   update norm\n"
                    << std::setw(19) << newton_alpha << "   newton alpha\n\n"
                    << std::endl;
            }
      ...
      if (std::fabs(res_norm) < abs_tol ||
          std::fabs(res_norm) < rel_tol * solution_norm)
        {
          pcout << std::endl
                << "   " << std::setw(19) << std::scientific << res_norm
                << " (converged in " << nonlin_iter << " iterations)\n\n"
                << std::endl;
      ...
      else if (outer_iter == max_outer_non_linear_iterations)
        {
          pcout << std::endl
                << "   " << std::setw(19) << std::scientific << res_norm
                << " (not converged in " << std::setw(3) << nonlin_iter
                << " iterations)\n\n"
                << std::endl;
          AssertThrow(false, ExcMessage("No convergence in nonlinear solver"));
        }
...
IMEXStepper<VEC>::set_functions_to_trigger_an_assert()
{
  ...
  residual = [](const double, const VEC &, const VEC &, VEC &) -> int {
    int ret = 0;
    AssertThrow(false,
                ExcPureFunctionCalled("Please implement residual function."));
    return ret;
  };
  ...
  solve_jacobian_system = [](const VEC &, VEC &) -> int {
    int ret = 0;
    AssertThrow(false,
                ExcPureFunctionCalled(
                  "Please implement solve_jacobian_system function."));
    return ret;
  };

  output_step = [](const double, const VEC &, const VEC &, const unsigned int) {
    AssertThrow(
      false, ExcPureFunctionCalled("Please implement output_step function."));
  };

  solver_should_restart = [](const double, VEC &, VEC &) -> bool {
    bool ret = false;
    AssertThrow(false,
                ExcPureFunctionCalled(
                  "Please implement solver_should_restart function."));
    return ret;
  };
  ...
  get_lumped_mass_matrix = []() -> VEC & {
    ...
    AssertThrow(false,
                ExcPureFunctionCalled(
                  "Please implement get_lumped_mass_matrix function."));
    return *y;
  };
  ...
  vector_norm = [](const VEC &vector) -> double { return vector.l2_norm(); };
}
