...
namespace AssimpInterface
{
                              ...
                              double                        tol)
  {
    ...
    cout << "The given file contains " << scene->mNumMeshes << " meshes. ";
    if (mesh_index == -1)
    ...
    for (int m = start_mesh; m < end_mesh; ++m)
      {
        ...
        cout << "Input mesh has " << n_vertices << " vertices and " << n_faces
             << " faces." << endl;
            ...
            if (mFaces[i].mNumIndices == GeometryInfo<dim>::vertices_per_cell)
              {
                ...
                for (unsigned int f = 0;
                     f < GeometryInfo<dim>::vertices_per_cell;
                     ++f)
                  {
              ...
              {
                cout << "Skipping face " << i << " of mesh " << m << ", it has "
    ...
    cout << "Triangulation has " << boundary_faces << " boundary cells."
         << endl;
    return true;
  }

  // Explicit instantiations
  template bool generate_triangulation(const string,
                                       Triangulation<1, 1> &,
                                       int,
                                       bool,
                                       double);
