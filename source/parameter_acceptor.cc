  ...
  if (out_filename != "")
    {
      ...
      std::string extension =
        out_filename.substr(out_filename.find_last_of(".") + 1);
...
/// string
template <>
std::shared_ptr<Patterns::PatternBase>
ParameterAcceptor::to_pattern<std::string>(const std::string &)
{
...
template <>
std::string
ParameterAcceptor::to_type<std::string>(const std::string &parameter)
{
  return parameter;
}
...
template <>
std::shared_ptr<Patterns::PatternBase>
ParameterAcceptor::to_pattern<double>(const double &)
{
...
template <>
std::shared_ptr<Patterns::PatternBase>
ParameterAcceptor::to_pattern<int>(const int &)
{
...
template <>
std::shared_ptr<Patterns::PatternBase>
ParameterAcceptor::to_pattern<unsigned int>(const unsigned int &)
{
...
template <>
std::string
ParameterAcceptor::to_string<unsigned int>(const unsigned int &entry)
{
  return std::to_string(entry);
}
...
template <>
unsigned int
ParameterAcceptor::to_type<unsigned int>(const std::string &parameter)
{
...
template <>
std::shared_ptr<Patterns::PatternBase>
ParameterAcceptor::to_pattern<bool>(const bool &)
{
...
#define MYP(dim)                                                 \
  template <>                                                    \
  std::shared_ptr<Patterns::PatternBase>                         \
  ParameterAcceptor::to_pattern<Point<dim>>(const Point<dim> &)  \
  {                                                              \
    return SP(new Patterns::List(Patterns::Double(), dim, dim)); \
  }                                                              \
                                                                 \
  template <>                                                    \
  std::string ParameterAcceptor::to_string<Point<dim>>(          \
    const Point<dim> &entry)                                     \
  {                                                              \
    return print(entry);                                         \
  }                                                              \
                                                                 \
  template <>                                                    \
  Point<dim> ParameterAcceptor::to_type<Point<dim>>(             \
...
#define MYV(type, sep)                                                        \
  template <>                                                                 \
  std::shared_ptr<Patterns::PatternBase>                                      \
  ParameterAcceptor::to_pattern<std::vector<type>>(const std::vector<type> &) \
  {                                                                           \
    type p;                                                                   \
    return SP(new Patterns::List(                                             \
      *to_pattern(p), 0, Patterns::List::max_int_value, sep));                \
  }                                                                           \
                                                                              \
  template <>                                                                 \
  std::string ParameterAcceptor::to_string<std::vector<type>>(                \
    const std::vector<type> &entry)                                           \
  {                                                                           \
    std::ostringstream s;                                                     \
    if (entry.size() > 0)                                                     \
      s << to_string(entry[0]);                                               \
    ...
    return s.str();                                                           \
                                                                              ...
                                                                              \
  template <>                                                                 \
  std::vector<type> ParameterAcceptor::to_type<std::vector<type>>(            \
    const std::string &parameter)                                             \
  {                                                                           \
  ...
  for (auto &it : parameters)
    {
      ...
      //      boost_parameter)) {}
        ...
        {}
                 ...
                 entry, pattern, boost_parameter))
        {}
      //      else if (to_boost_any<std::vector<std::vector<bool> > >(entry,
      //      pattern, boost_parameter)) {}
      else
        {
          AssertThrow(false, ExcNotImplemented());
        }
