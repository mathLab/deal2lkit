...
/**
 * A deal2lkit wrapper for the nanoflann library, used to compute
 * distances from collection of points. This function uses nanoflann
 ...
 * functionalities of the nanoflann library.
 */
template<int dim>
...
public:
   ...
   * to pass them later to the function by calling the set_points()
   * method.
   */
  ...
  /**
   * Adaptor class used internally by nanoflann. This is the class
   * that actually stores a reference to the vector of points.
   */
  struct PointCloudAdaptor
  {
     ...
     * immediate value, the "if/else's" are actually solved at compile
     * time.
     */
   ...
   *
   * If the adaptor is empty, throw an exception. Call the function
   * set_points() to initialize a new adaptor, or call the constructor
   * with a valid set of points.
   *
   ...
   * metho, you can call the value() method to compute the minimum
   * distance between an evaluation point and the collection of points
   * you passed to this method.
   *
