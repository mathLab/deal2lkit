...
namespace SacadoTools
{
  ...
  number
  val(const Sacado::Fad::DFad<number> &n)
  {
    return n.val();
  }
  ...
  template <typename number>
  std::vector<number>
  val(const std::vector<Sacado::Fad::DFad<number>> v)
  {
  ...
  template <typename number>
  std::vector<double>
  to_double(const std::vector<number> &v)
  {
