 ...
 * the necessity to instantiate and initialize new vectors of the right type and
 * size everytime you need to access finite element solutions.
 *
 * Before anything sensible can be extracted by this class, you have to call
 * first the cache_local_solution_vector() function, and then one of the
 * reinit() functions.
 *
 * If you try to access the values before a cache_local_solution_vector is
 * called, an exception will be thrown.
 *
 * This function handles correctly also Sacado types (both first order and
 * second order derivatives), and automatically initializes their derivative.
 * This allows you to access derivatives w.r.t. to local degrees of freedom?
 *
 */
...
public:
   ...
   *
   * Before you call this function, you have to make sure you have previously
   * called the function cache_local_solution_vector with the same name you give
  ...
  get_values(const std::string &prefix, const Number dummy)
  {
    ...
    if (!cache.have(name))
      {
        cache.add_copy(std::vector<std::vector<Number>>(
  ...
  /**
   * Return the laplacian of the named cached solution vector. Vector value
   * case.
   *
   * Before you call this function, you have to make sure you have previously
   * called the function cache_local_solution_vector with the same @p prefix you give here
