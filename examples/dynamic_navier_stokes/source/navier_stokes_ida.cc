...
template <int dim>
NavierStokes<dim>::NavierStokes (const MPI_Comm &communicator)
  :
...
template <int dim>
void NavierStokes<dim>::setup_dofs (const bool &first_run)
{
...
template <int dim>
void NavierStokes<dim>::update_constraints (const double &t)
{
...
template <int dim>
void NavierStokes<dim>::assemble_jacobian_matrix(const double t,
                                           const VEC &solution,
                                           ...
                                           const double alpha)
{
  ...
  std::vector<Tensor<1,dim> >          grads_phi_p(dofs_per_cell);
            ...
            for (unsigned int k=0; k<dofs_per_cell; ++k)
              {
                ...
                grads_phi_p[k] = fe_values[pressure].gradient(k,q_point);
              }
                ...
                for (unsigned int j=0; j<dofs_per_cell; ++j)
                  {
                    ...
                    cell_prec(i,j) += (
                                        ...
                                        +
                                        (1./mu)*grads_phi_p[i]*grads_phi_p[j]
                                        +
...
template <int dim>
int NavierStokes<dim>::residual (const double t,
                           const VEC &solution,
                           const VEC &solution_dot,
                           VEC &dst)
{
...
template <int dim>
shared_ptr<VEC> NavierStokes<dim>::create_new_vector () const
{
...
template <int dim>
unsigned int NavierStokes<dim>::n_dofs() const
{
  return dof_handler->n_dofs();
}

template <int dim>
void NavierStokes<dim>::output_step(const double t,
                              const VEC &solution,
                              const VEC &solution_dot,
...
template <int dim>
bool NavierStokes<dim>::solver_should_restart (const double t,
                                         const unsigned int step_number,
                                         const double h,
                                         VEC &solution,
                                         VEC &solution_dot)
...
template <int dim>
int NavierStokes<dim>::setup_jacobian (const double t,
                                 const VEC &src_yy,
                                 const VEC &src_yp,
                                 const VEC &,
                                 const double alpha)
...
template <int dim>
int NavierStokes<dim>::solve_jacobian_system (const double t,
                                        const VEC &y,
                                        const VEC &y_dot,
                                        const VEC &,
                                        const double alpha,
...
//          n_iterations = ( solver_control_refined.last_step());
//        }
...
template <int dim>
VEC &NavierStokes<dim>::differential_components() const
{
...
template <int dim>
void NavierStokes<dim>::set_constrained_dofs_to_zero(VEC &v) const
{
